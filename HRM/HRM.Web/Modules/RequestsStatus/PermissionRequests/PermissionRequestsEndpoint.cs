...
namespace HRM.RequestsStatus.Endpoints
{
    ...
    [RoutePrefix("Services/RequestsStatus/PermissionRequests"), Route("{action}")]
    ...
    public class PermissionRequestsController : ServiceEndpoint
    {
        ...
        public RetrieveResponse<MyRow> Retrieve(IDbConnection connection, RetrieveRequest request)
        {
            ...
            PermissionRequestsRow VacancyRequestsRow = connection.List<PermissionRequestsRow>().FirstOrDefault(x => x.Id == ID);
            int? StatusId = VacancyRequestsRow.StatusId;
            short? IsAcceptedbyHr = VacancyRequestsRow.IsAcceptedbyHr;
            if (StatusId == 1 && IsAcceptedbyHr == 1) // Approved and closed 
            {
